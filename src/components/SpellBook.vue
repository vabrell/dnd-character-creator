<template>
  <div ref="spellbook">
    ...
    <div class="container">
      <input
        class="w-full text-xl py-2 px-4 border-b-4 border-grey-300 focus:border-red-200"
        v-model="filter"
    ...
    <div class="grid grid-cols-3 gap-4 mt-2">
      <div
        class="pr-2"
        :class="{
          'col-span-3': !spellDescription,
          'shadow-r-lg': spellDescription
        }"
      >
        <span v-if="!spellList">Opening spellbook...</span>
        <ul>
          <li
            class="text-lg hover:bg-red-200 hover:text-red-600 py-1 px-2 rounded my-1"
            :class="{
              'bg-red-200': selected(spell.index),
              'text-red-600': selected(spell.index)
            }"
          ...
          >
            <router-link
              ...
              :to="`/spellbook/${spell.index}`"
              @click.native="updateSelectedSpell"
              >{{ spell.name }}</router-link
            >
          </li>
        </ul>
      </div>
      <div class="col-span-2 pt-2" v-if="spellDescription">
        ...
        <p>
          ...
          <span
            class="bg-red-500 text-white rounded-full py-1 px-2"
            v-if="spellDescription.concentration"
            >C</span
          >
        </p>
        <p><b>Components:</b> {{ spellDescription.components.join(', ') }}</p>
        <p v-if="spellDescription.material">
          <b>Material:</b> {{ spellDescription.material }}
        </p>
        ...
        <h4 v-if="spellDescription.higher_level" class="text-xl font-semibold">
          Higher level
        </h4>
